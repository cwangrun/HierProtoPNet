...
def push_prototypes(dataloader,  # pytorch dataloader (must be unnormalized in [0,1])
    ...
    for push_iter, (search_batch_input, search_y, img_name) in enumerate(dataloader):
        ...
        with torch.no_grad():
            ...
            protoL_input_torch, proto_dist_torch = prototype_network_parallel.module.push_forward_middle(search_batch)
        ...
        for img_index, img_y in enumerate(search_y):
            ...
            img_labels = torch.where(img_y == 1)[0]         ################################
            for img_label in img_labels:
                class_to_img_index_dict[img_label.item()].append(img_index)
    ...
    for j in range(n_prototypes):
        ...
        for push_idx in sorted_idx:
            ...
            with torch.no_grad():
                protoL_feat_torch, proto_dist_torch = prototype_network_parallel.module.push_forward_middle(img_torch)
            ...
            proto_act_img_j = np.exp(-proto_dist_img_j / 256.0)
            proto_act_img_j = (proto_act_img_j - proto_act_img_j.min()) / (proto_act_img_j.max() - proto_act_img_j.min())
            ...
            highest_index = np.array(np.unravel_index(np.argmax(proto_act_img_j), proto_act_img_j.shape)) * int(upsampled_act_img_j.shape[0] / proto_act_img_j.shape[0])
            ...
            rescaled_act_img_j = np.power(rescaled_act_img_j, 4.0)
            ...
            #############################################################################################
            mask_ori = (rescaled_act_img_j >= 0.65).astype(np.uint8)
            contours, hierarchy = cv2.findContours(mask_ori, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)
            circle_ori = np.zeros_like(mask_ori)
            for i in range(len(contours)):
                for y, x in contours[i].squeeze(1):
                    circle_ori[x, y] = 1
            circle_ori = ndimage.binary_dilation(circle_ori.astype(int), structure=np.ones((6, 6)))
            original_img_j[circle_ori != 0, :] = (1, 1, 0)
            #############################################################################################
...
def imsave_with_bbox(fname, img_rgb, bbox_height_start, bbox_height_end, bbox_width_start, bbox_width_end, highest_index, color=(0, 255, 255)):
    ...
    # plt.imsave('fname.jpg', img_rgb_float, vmin=0.0, vmax=1.0)