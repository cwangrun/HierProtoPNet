...
class DenseNet_features(nn.Module):
    ...
    def __init__(self, growth_rate=32, block_config=(6, 12, 24, 16),
        ...
        # Lateral layers
        self.latlayer1 = nn.Conv2d(blockwise_num_features[-2], 256, kernel_size=3, stride=1, padding=1)  ##### kernel_size=1, padding=0
        self.latlayer2 = nn.Conv2d(blockwise_num_features[-3], 256, kernel_size=3, stride=1, padding=1)
        self.latlayer3 = nn.Conv2d(blockwise_num_features[-4], 256, kernel_size=3, stride=1, padding=1)
        #####################################################################################
    ...
    def forward(self, x):
        ...
        p5 = self.toplayer(layerwise_features[-1])     # 1*1, ->256
        p4 = self._upsample_add(p5, self.latlayer1(layerwise_features[-2]))    # 1*1, ->256
        p3 = self._upsample_add(p4, self.latlayer2(layerwise_features[-3]))    # 1*1, ->256
        # p2 = self._upsample_add(p3, self.latlayer3(layerwise_features[-4]))    # 1*1, ->256

        # Smooth
        p4 = self.smooth1(p4)   # 3*3, ->256
        p3 = self.smooth2(p3)   # 3*3, ->256
        # p2 = self.smooth3(p2)   # 3*3, ->256
